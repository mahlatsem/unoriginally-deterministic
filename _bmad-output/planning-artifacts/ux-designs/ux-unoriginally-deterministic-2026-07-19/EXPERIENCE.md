...
| "Same code, same structure — re-run anytime, free." | "Refreshing your analysis…" (implies nondeterminism) |
...
**Determinism copy names structure, not pixels.** "Same code, same *structure*" is what the toolkit can guarantee; a graph layout may arrange the same structure differently between runs unless layout itself is made deterministic. Either the architecture owns pixel-stable layout as an explicit requirement, or the copy claims structure — the spine picks structure, so the promise stays true either way.
...
| Stale snapshot | View canvas + editor | Files changed since the view was drawn: header states it ("drawn 14:02 · source changed since — re-run to re-derive"). No auto-re-derive mid-reasoning. A jump-to-source into a changed file says so rather than landing confidently on a line that may have moved. |
...
| Tier | Contains | Why |
|---|---|---|
| **Gate slice** *(subset of Phase A)* | Dependency map only · anchor popover · jump-to-source · unparsed + unresolved rendering · extraction incl. inclusion resolution | Exactly what the reliability gate runs against — one perspective, not six. Inclusion resolution is inside the slice because a gate passed against unresolved includes might not transfer to the real estate. |
...
**The trail: temporal, not spatial (Phase B).** Every drill and perspective switch pushes a step onto the view trail; Back / Forward restore the exact prior step — scope, perspective, *and* selection. `Esc` retreats *structurally* (up a grain); Back retreats *temporally* (to where your reasoning was, even if that was a different perspective two grains down). Trail steps whose anchors no longer exist after a re-derive restore with the same stale notice rather than silently rendering. The trail is history behind ordinary navigation keys, so the canvas always shows exactly one thing — see *Inspiration & Anti-patterns* for what this deliberately avoids.
...
|---|---|---|
| **System** | a **compilation/deployment unit** — the things that depend on each other | **Dependency map** · **Sequence diagram** · **Reach tracer** |
...
- **Dependency map** (system) — the orientation perspective, and the one a user meets first. Opens fit-to-canvas at whole-system scope so the shape of the estate is visible before drilling. Nodes: compilation units and the sources they include. Edges: **static reference** — one unit naming another (invocation, inclusion, declared resource access) — category-coloured with a legend. Collapse-by-subtree for density; never silently truncates (it says what it collapsed).
...
- **Reach tracer** (system) — the blast-radius perspective, most directly serving "I have to change this without breaking the business." Radiates from one unit through what it **statically** reads, writes, and invokes; edge kind (read / write / invoke) is first-class because *writes* carry the risk. Depth is user-controlled and stated ("reach depth 2 — 3 further hops collapsed"), never silently bounded, and the same header breath reports unresolved targets and holes. Never "everything it touches" — unearnable wherever dynamic dispatch or parse holes exist, and the rejected-framings list bans exactly that completeness claim.
...
|---|---|---|
| System node — compilation unit | program, copybook, data file | module / source file |
...
| Static reference edge | `CALL 'LIT'`, `COPY`, `SELECT`/`FD` | `import`, direct call, re-export |
