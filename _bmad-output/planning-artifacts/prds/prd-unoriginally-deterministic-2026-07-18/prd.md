...
**Updated 2026-07-19** to reconcile with the finalized UX design spines ([DESIGN.md](../../ux-designs/ux-unoriginally-deterministic-2026-07-19/DESIGN.md) · [EXPERIENCE.md](../../ux-designs/ux-unoriginally-deterministic-2026-07-19/EXPERIENCE.md)), which closed OQ-3, expanded FR-5, added copybook expansion and two further honest states to FR-1, and introduced the build phasing in §5.2. **Division of authority:** this PRD owns *what must be true*; the spines own *how it looks and behaves* and win on any UX question. Where they conflicted on scope, they were reconciled on 2026-07-19 and this PRD now governs.
...
Extraction must also report the limits of what it could establish, and those limits fall into three general categories — **parse-failure regions**, **dynamic dispatch**, and **unresolvable inclusion**. They are properties of grammars, not of COBOL: COBOL supplies v1's instances, and every language added later supplies its own. The categories and their required treatment therefore belong to the model, so adding a language does not reopen them (the per-language mapping lives in `EXPERIENCE.md`'s language-profile table).

#### FR-1: COBOL source extraction
The toolkit can parse COBOL source files (base tree-sitter grammar + community extensions), resolve copybook inclusion, and expose a queryable syntax representation to the extension contract's primitives.
...
|---|---|---|
| System | a compilation unit | Dependency map · Sequence diagram · Reach tracer |
...
- **Not pixel-stable view layout at v1.** Determinism is claimed over derived *structure* — the same code yields the same elements and relationships on any machine, every run. Whether that structure is also *arranged* identically depends on the layout algorithm, and v1 promises nothing there, so product copy claims structure, never "the same picture." Architecture inherits this as an explicit fork: adopt deterministic layout, or keep the narrower claim.
...
|---|---|---|
| **Gate slice** *(subset of Phase A — what OQ-1/SM-2 actually runs against)* | COBOL extraction **including copybook expansion** · provenance-enforcing primitives · **dependency map only** · click-to-source · unparsed + unresolved rendering | Matches the SM-2 protocol's thin vertical slice (`addendum.md`), narrowed to the dependency map. Copybook expansion is *inside* the slice because real COBOL is copybook-saturated — a gate passed against non-expanded extraction might not transfer to the actual dogfooding target. |
...
- **Grain** — the level a scope sits at: **system** (a compilation unit), **component** (a named callable), **unit** (control flow within one callable). Drilling moves between grains.
- **Gate slice** — the minimal build the reliability gate runs against: extraction with copybook expansion, provenance-enforcing primitives, the dependency map, and click-to-source (§5.2). Deliberately smaller than Phase A.
