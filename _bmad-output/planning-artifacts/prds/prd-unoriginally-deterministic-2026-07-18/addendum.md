...
- **Asymmetry to flag for UX/architecture:** starter tools silently update whenever the extension updates (Marketplace release); molded tools are frozen until the user or Claude Code re-mold them (this is the intended FR-6 guarantee). A user should probably be able to *feel* that difference somewhere in the UI — open item, not resolved here, flagged for the UX pass.
...
1. **Build the thin vertical slice first** — COBOL extraction, the provenance-enforcing primitives, and one starter view (FR-1, FR-4, FR-5). The experiment tests the extension contract (FR-3/FR-4), not the whole product.
