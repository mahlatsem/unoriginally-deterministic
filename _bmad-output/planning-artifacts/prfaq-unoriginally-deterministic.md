...
- **THE crack — molding reliability is unproven (Q1/Q3).** Everything rests on the toolkit narrowing the AI's job enough that Claude Code (the builder's own assistant) reliably emits correct, plug-in tools. Until a prototype demonstrates that, it's a hope. **What it takes to address:** build a thin vertical slice — tree-sitter extraction + provenance-enforcing primitives + one view — then have Claude Code generate a custom tool across several independent tasks and measure how many plug in and pass the trust checks *unedited*. That is the go/no-go experiment, and it should come before any broad build. Cross-assistant portability is not a goal to prove — the toolkit is assistant-agnostic by design, so other users are free to validate a different assistant on their own.
...
**Bottom line:** The thinking is sharp, the positioning is honest, and the scope is realistic. The idea deserves to be built — but build the *reliability prototype* first. If Claude Code can reliably produce a working, provenance-carrying tool against your contract without hand-holding, you have a product. If it can't, you have GT's adoption problem in a new editor, and better to know in a month than a year.
...
**KEY BUILDABILITY INSIGHT (the session's best):** Trust is enforced STRUCTURALLY, not by convention — **provenance (source span) is a REQUIRED argument of the drawing primitives.** No source → no node/edge emitted. A lazy AI-generated (or rushed hand-written) tool can't silently break the trace-to-source guarantee; it just produces nothing. This is what makes "make it easy for AI *and* human to get close" concrete: the correct provenance-carrying path is the ONLY path. This is the make-or-break design surface and the answer to Q3(customer), Q7(customer), Q1/Q3(internal) simultaneously.
...
- **ARCHITECTURE DECISION — the toolkit bundles NO AI.** It is deterministic code end to end: a runtime + starter tools + a *moldable framework designed to be extended by whatever AI assistant the engineer already uses* (Claude Code / Copilot / Cursor / etc.). The product's job is to make analysis tools — whether AI-authored or hand-written — deterministic, durable, and shareable — NOT to be the AI. Hand-writing a tool is always possible; AI just makes writing it faster. Benefits: removes model/key dependency and liability (nothing AI running in a regulated repo), rides existing AI-assistant adoption, more durable than any bundled model (AI churns; a clean extension contract + deterministic primitives endure). This supersedes the earlier "BYO-key inside the product" framing. "Free, every run" = RUNNING tools; the one-time build cost lives entirely in the engineer's own assistant, outside the product.
- **NEW load-bearing design surface (Internal FAQ):** because the toolkit does NOT bundle AI, "*easy* to mold, with AI or by hand" rests entirely on the quality of the **extension contract** — the API, conventions, templates, and deterministic primitives that an arbitrary LLM can pattern-match to emit a correct tool, and that a human can just as readily write against directly. Using AI is recommended, especially for speed, but the contract must not assume it. This is now the make-or-break engineering work. Trust checks (audit-the-analyzer / trace-every-element-to-source) remain the safety net.
